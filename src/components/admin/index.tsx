  ...
  return (
    <>
      {currentUser ? (
        <>
          {meta}
          <div className="h-screen flex overflow-hidden bg-white" style={{ zIndex: 10000 }}>
            <button
              onClick={() => {
    ...
    <div className="py-10 px-4 bg-white min-h-screen mb-10 lg:mb-0">
      ...
      <div className="overflow-x-scroll">
        {data?.objects?.length > 0 ? (
          <table className="bg-white divide-y divide-gray-300 w-full mb-16 table-auto overflow-scroll">
            <thead>
              <tr>
                ...
                <tr key={object.id}>
                  {Object.entries(object).map(([key, value], index) => (
                    <td key={index} className="whitespace-wrap px-4 py-4 text-sm text-gray-700">
                      ...
                      {key?.includes("Type") && <span className=" rounded-full bg-green-100 px-2.5 py-1 text-xs">{value as string}</span>}
                      {!key?.includes("Id") &&
                        !key?.includes("Type") &&
          ...
          <div className="w-[98%] mx-auto mt-10 rounded p-4 border border-gray-200 text-gray-500">No Data</div>
        )}
      </div>
    </div>
  );
              ...
              {Object?.entries(data).map(([key, value], index) =>
                typeof value === "object" ? (
                  <div key={index} className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
                    ...
                    <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 whitespace-pre-wrap">TBD</dd>
                  </div>
                ) : (
                  <div key={index} className="py-4 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-5">
                    <dt className="text-sm font-medium text-gray-500">{key}</dt>
